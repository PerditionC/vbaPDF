Attribute VB_Name = "LibInflate_rfc1951"
...
Private Enum ExtraBitsFunction
    ExtraBits_Distance
    ExtraBits_Length
End Enum
...
' Returns full length value from 3 to 258, additionally advances byteOffset and bitOffset correspondingly (by number of extra bits read)
' Note: code 28 specially handled when building lengthTree so value is 258 not 259
...
Private Sub buildStaticCodeTree(ByRef tree() As codeNode, ByVal extraBitsFunc As ExtraBitsFunction, Optional ByVal initialCodeValue As Long = 0)
    Dim bytes(0 To 3) As Byte
    bytes(0) = 255: bytes(1) = 255: bytes(2) = 255: bytes(3) = 255  ' max code's distance has all 1s in its extra bits
    Dim currentMax As Long: currentMax = initialCodeValue   ' lowest code (initial starting code) value - 1
    ' for distance code, the first 5 bits are code value for distance, 0-31 + extra bits
    ' for length code, encoded value - 257 so in range of 0 to 28
    Dim code As Long
    For code = LBound(tree) To UBound(tree) ' either 0 to 31 or 0 to 28
        Set tree(code) = New codeNode
        If extraBitsFunc = ExtraBitsFunction.ExtraBits_Distance Then
            tree(code).extraBitsLen = distanceExtraBits(code)
        Else ' If extraBitsFunc = ExtraBitsFunction.ExtraBits_Length Then
            tree(code).extraBitsLen = lengthExtraBits(code)
        End If
        
        Dim byteOffset As Long, bitOffset As Long
        byteOffset = 0: bitOffset = 0
        Dim extraBits As Long
        extraBits = nBITS(tree(code).extraBitsLen, bytes, byteOffset, bitOffset)
        tree(code).minValue = currentMax + 1 '+ 0          ' this code's distance begins 1 greater than last code's max value
        tree(code).maxValue = currentMax + 1 + extraBits   ' max distance value used is 32768 for distance, 258 for length
        currentMax = tree(code).maxValue                   ' update for next iteration of loop
        ...
        Else
            Debug.Print "Length";
        End If
        Debug.Print "Code=" & code & " (" & tree(code).extraBitsLen & " bits) " & tree(code).minValue & "-" & tree(code).maxValue
    Next
End Sub
Private Sub buildDistanceTree()
    buildStaticCodeTree distanceTree, ExtraBits_Distance
End Sub

Private Sub buildLengthTree()
    buildStaticCodeTree lengthTree, ExtraBits_Length, initialCodeValue:=2
...
Private Function buildHuffmanTree(ByRef lengths As Variant, ByRef minBitLen As Integer, ByRef maxBitLen As Integer) As huffmanCode()
    ...
    For ndx = minCode To maxCode
        ...
        ' ensure index initialized as unused, if Class module left as Nothing so this isn't needed
        huffmanTree(ndx).bitLen = -1
    Next ndx
    minBitLen = minValue
    maxBitLen = maxValue
    ...
    For curBitLength = minValue To maxValue
        ...
        For ndx = minCode To maxCode
            ...
            If (CLng(lengths(ndx)) = curBitLength) And (curBitLength > 0) Then
                ...
                'Set huffmanTree(ndx) = New huffmanCode ' only needed if huffmanCode is a Class module and not a Type
                ...
                ' left side of tree
                curCode = curCode + 1
            ' Else do nothing as may have been set with different bit length
            End If
            DoEvents
        Next ndx
    Next curBitLength
...
Private Sub buildReverseHuffmanMapping(ByRef mapHuffmanCodeToValue() As Integer, ByRef huffmanCodes() As huffmanCode, ByVal maxBits As Integer)
    ...
    For ndx = LBound(huffmanCodes) To UBound(huffmanCodes)
        If huffmanCodes(ndx).bitLen >= 0 Then 'Not huffmanCodes(ndx) Is Nothing Then
...
'Private LengthTree(0 To (285 - 257)) As CodeNode    ' (0 to 28)
    ...
    Do ' loop until we reach end of block code, 256
        ...
        If literalOrLength < 256 Then ' is it a literal value
            If (outputOffset > UBound(oBytes)) Then ReDim Preserve oBytes(0 To UBound(oBytes) * 2 - 1) ' expand buffer as needed
        ...
        Else ' is it length, distance value, > 256
            ...
            If ((outputOffset + literalOrLength - 1) > UBound(oBytes)) Then ReDim Preserve oBytes(0 To UBound(oBytes) * 2 - 1) ' expand buffer as needed
            Dim i As Long
...
Public Function inflate2(ByRef cbytes() As Byte, ByRef oBytes() As Byte, _
                         ByRef inputOffset As Long, ByRef outputOffset As Long, _
                         Optional ByVal EstimatedOutputSize As Long = -1, _
                         Optional ByVal preAllocated As Boolean = False) As Boolean
    
    ' if not allocated, then assume 4X compressed size (will double allocation as needed)
    ' Note: we shrink oBytes to size of decompressed data unless requested not to
    On Error Resume Next ' if error getting bounds then assume unallocated
    If UBound(oBytes) < LBound(oBytes) Then
        If EstimatedOutputSize <= 0 Then EstimatedOutputSize = ((UBound(cbytes) - LBound(cbytes)) * 4) - 1
        ReDim oBytes(0 To EstimatedOutputSize)
    End If
    On Error GoTo errHandler
...
'''
    ...
    Do
        ...
        Select Case blockCompressionType
            Case ZlibCompressionType.NoCompression
                ...
                ' hack
                If ((copyLen = 0) And (negatedCopyLen = 65535)) Then
                    lastBlock = True
                End If
                ' end hack
                If (copyLen <> negatedCopyLen) And Not lastBlock Then
                    Debug.Print "Error invalid uncompressed length value!"
                    Stop
                    Exit Function
                End If
                ' copy LEN bytes of data to output
                If ((outputOffset + copyLen) > UBound(oBytes)) Then ReDim Preserve oBytes(0 To UBound(oBytes) * 2 - 1) ' expand buffer as needed
    ...
    ' trim output buffer to fit, but only if we allocated it
    If Not preAllocated Then ReDim Preserve oBytes(0 To outputOffset - 1)
    inflate2 = True
